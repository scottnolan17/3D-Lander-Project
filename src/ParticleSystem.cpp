...
// Rotate the new thrust by the given angle, and update the thrust force
//
void ThrusterForce::incrementThrust(ofVec3f newThrust, float angle) {
	ofVec3f temp = newThrust;
	temp.x = -1*(newThrust.x * cos(glm::radians(angle)) - newThrust.z * sin(glm::radians(angle)));
	temp.z = newThrust.x * sin(glm::radians(angle)) + newThrust.z * cos(glm::radians(angle));
	thrust = temp;
}
